...
export async function getJourneys(stops: string[], opt: JourneysOptions, client: HafasClient): Promise<TreeMatrixPair | ZugError> {
    ...
    let firstJourneysResponse = await hafasClient.journeys(stops[0], stops[1], opt);
    //console.log(firstJourneysResponse)
    if (firstJourneysResponse.journeys === undefined) {
        return "error"
    }
    ...
    for (let i = 1; i < stops.length - 1; i++) {
        ...
        //console.log(opt)
        let journeysResponse = await hafasClient.journeys(stops[i], stops[i + 1], opt);
        if (journeysResponse.journeys === undefined) {
            return "error"
        }
...
async function requestMoreJourneys(laterRef: string, from: string, to: string, opt: JourneysOptions, latest: Date): Promise<Journey[]> {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        //console.log(opt)
        let laterResult = await hafasClient.journeys(from, to, opt)
